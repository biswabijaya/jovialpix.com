...
if (isset($_POST['action']) and $_POST['action']=='update') {
  ...
  if ($result=mysqli_query($mysqli,"UPDATE blog set title='$title', status='$status', summary='$summary', body='$body' where id=$id")) {
    header("Location: blogs?msg=UpdateSuccess");
  } else {
    header("Location: blogs?msg=UpdateNotSuccess");
  }
}
...
if (isset($_POST['action']) and $_POST['action']=='addpost') {
  ...
  if ($result=mysqli_query($mysqli,"INSERT into blog VALUES (NULL, '$userid', '$title', '$summary', '$body', '$headerimage', '0', '$datetime')")) {
    if($result = mysqli_query($mysqli, "SELECT * From users WHERE id=$userid "))
      ...
      $mail ->SetFrom("admin@jovialpix.com","JovialPix Blogs");
    ...
    header("Location: blogs?msg=AddPostSuccess");
  } else {
    header("Location: blogs?msg=AddPostNotSuccess");
  }
}
?>
